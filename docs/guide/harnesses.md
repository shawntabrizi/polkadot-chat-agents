...
| `POST /send { chat_id, text?, file_path?, reply_to?, edit_of? }` | publishes a reply, file, or edit; returns `{ success, message_id }`. `file_path` must identify a file already in that same chat's vault. `reply_to` and `edit_of` are mutually exclusive. A T3ams file may include a caption and reply target, but never `edit_of`. **Live replies:** when a turn runs long, bot-core posts a thinking placeholder; the first plain send retires it to a short status receipt and publishes the answer as a new message (so the returned `message_id` is the answer's). `edit_of` frames are throttled and coalesced server-side, so a harness may stream freely. `GET /health` advertises the actual `live` capability. |
