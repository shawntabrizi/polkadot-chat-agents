...
| Variable | Default | Purpose |
|---|---|---|
| `BOT_AI_WORKSPACE` | `BOT_STATE_DIR/workspace` (deploy: `/workspace`) | Non-secret directory the agent works in, separate from state. A direct bot's operator-owned `PERSONA.md` lives here; PCA creates its commented template once and never overwrites it. **gen (direct)** |
...
For Codex, OpenCode, and Kimi, PCA normally owns a generated `AGENTS.md` in
the active workspace and identifies it with
`<!-- generated by pca: operator context -->`. An unmarked operator file is
never replaced: PCA writes `.pca/OPERATOR-CONTEXT.md`, logs
...
| `BOT_COLOR_SWATCH` | unset | `1` turns on colour swatches (the pcdcolor demo bot). In a DM, a message with a hex code (`#RRGGBB`, `#RGB`, or a bare 6-digit word that mixes digits and letters such as `ff8800`) is answered with a 512×512 PNG swatch, caption `#RRGGBB — <CSS name if known>`, and no brain turn. A PNG image (8-bit RGB or RGBA) or a JPEG image (decoded by `jpeg-js`) is answered with a swatch of its dominant colour, caption `Dominant #RRGGBB · average #RRGGBB`; other formats get a text reply. A brain answer that contains a hex code goes out as a swatch whose caption is `#RRGGBB — <the answer's **bold** name>` and the rest of the answer. The swatch is a spec 0012 attachment on Bulletin (one message; on devnet the bot needs `BOT_BULLETIN_AUTHORIZER=//Eve`); without Bulletin the caption goes out as text. Group turns are unchanged. |
...
| `BOT_HEARTBEAT_MS` | 120000 | Subscription liveness heartbeat interval. Each heartbeat is one Statement Store submission (30 per hour at the default). |
