...
const fill = (x, n) => new Uint8Array(n).fill(x);
const GROUP = "0b7e2c1a-0000-4000-8000-00000000c0de";
// Account order matters for the tie rules (lower signer wins): alice < bot.
const ACCOUNTS = { alice: "a1".repeat(32), bob: "b0".repeat(32), bot: "b7".repeat(32), carol: "c4".repeat(32), dave: "d5".repeat(32) };

const makeWorld = ({ start = 1_790_000_000_000 } = {}) => {
  const clock = { t: start };
...
const text = (id, t, body) => encodeOpaqueTextMessage({ messageId: id, timestamp: t, text: body });
const texts = (res) => res.flatMap((r) => r.messages ?? []).map((x) => x.message.text);
...
test("admin bot admits by invite: policy 2 at once (state + welcome), policy 1 pending, a bad proof or used-up invite rejected", async () => {
  ...
  assert.equal(bot.groups.acceptsJoinOpener(carol.account, proofText(carol.account)), true, "the opener's capability is checked before the chat is accepted");
  assert.equal(bot.groups.admits(carol.account, () => false), true);
  assert.equal(bot.groups.acceptsJoinOpener(ACCOUNTS.dave, proofText(carol.account)), false, "a proof is bound to the joiner's account");
