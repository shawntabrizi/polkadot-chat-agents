...
import { createMeter, formatPas, parsePlancks } from "../lib/meter.mjs";
...
const fakeChain = ({ balances = {}, failReads = false, dispatchFails = false } = {}) => {
  const state = { balances: new Map(Object.entries(balances).map(([a, p]) => [a, BigInt(p) * RATIO])), calls: [], mapped: 0 };
...
const setup = (chainOptions = {}, meterOptions = {}) => {
  ...
  return { chain, sent, logs, meter, user: reviveAddress(`0x${PEER}`) };
};
...
test("enough balance: the brain runs, then the price is charged and a reference reports what is left", async () => {
  const { meter, sent, chain, user, logs } = setup();
  chain.state.balances.set(user, (PAS * 3n / 10n) * RATIO); // 0.3 PAS
  const gate = await meter.beforeTurn(PEER, text("hello"));
  assert.deepEqual(gate, { run: true, charge: true });
  const result = await meter.afterTurn(PEER);
  assert.equal(result.remaining, PAS * 2n / 10n);
  assert.equal(chain.state.calls[0].pair, operator, "the bot's own wallet signs the charge");
  assert.equal(chain.state.calls[0].calldata, meterCalldata.charge(user, (PAS / 10n) * RATIO), "the price is scaled into contract units");
  assert.deepEqual(sent, [{ type: "reference", peer: PEER, ref: { chainId: GENESIS, hash: `0x${"cd".repeat(32)}`, status: 1, block: 123, note: `balance: ${PAS * 2n / 10n}` } }]);
  await meter.beforeTurn(PEER, text("again"));
  await meter.afterTurn(PEER);
  assert.equal(chain.state.mapped, 1, "the operator account is mapped once, not per charge");
  assert.ok(logs.some((l) => l.event === "BOT_METER_CHARGED" && l.remaining === String(PAS / 10n)));
});
...
test("a charge that fails is logged; one that failed on chain is reported as status 3", async () => {
  ...
  const failing = setup({ dispatchFails: true });
