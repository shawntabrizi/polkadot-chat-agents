...
export const createAgentRuntime = ({
  ...
  buildArgs,     // ({ prompt, model, resume, effort, attachmentDir, outputDir, workingDirectory }) -> argv
  // Optional per-turn, non-secret environment additions generated by the
  // selected runner (for example, an attachment-scoped engine permission).
  buildTurnEnvironment = null,
  ...
  // Direct-agent CLIs can receive the real absolute output path in their
  // environment and permission profile. Model-visible file references stay
  // relative to the selected workspace: Codex's native apply_patch contract
  // rejects absolute paths, while relative `../…` paths retain the protected
  // root-owned handoff directory without a writable-workspace symlink.
  const outputPathForPrompt = (turnCwd, outputDir) => {
    const relative = path.relative(turnCwd, outputDir);
    if (!relative) return ".";
  ...
  const runEngine = (peerHex, userText, onAction = null, onPartial = null, cwd = workspace, job = null, outputDir = null, sessionKey = peerHex, attachmentDir = null) => new Promise((resolve) => {
    ...
    try {
      ...
      const turn = {
        ...
        attachmentDir,
        // A transport-owned per-turn handoff directory. A runner can grant
        // it as a separate write capability without widening the workspace.
        outputDir,
        // Runners receive the selected project cwd so their native policy
        // can make that directory the tool root for this turn.
        workingDirectory: cwd,
      };
      const argv = buildArgs(turn);
      ...
      const turnEnvironment = buildTurnEnvironment ? buildTurnEnvironment(turn) : null;
      const env = {
        ...childEnv,
        ...(turnEnvironment ?? {}),
      ...
      try {
        const result = await queueEngineTurn(peerHex, async (job) => {
          ...
          try {
            ...
            // The real output directory remains an engine-only capability
            // context. Render a relative path for the model so Codex can use
            // its native patch tool without accepting an absolute file name.
            const promptMessage = outputDir == null
              ? messageForPrompt
              : { ...messageForPrompt, outputDir: outputPathForPrompt(turnCwd, outputDir) };
