...
test("a persona registers its identity and every device account, and never exposes a secret", () => {
  ...
  assert.deepEqual(Object.keys(JSON.parse(json)).sort(), ["account", "bulletinAccount", "chatPublicKey", "devices", "name", "registration", "username"]);
  assert.deepEqual([JSON.parse(json).username, JSON.parse(json).registration], ["bob", null], "on the mock network the username is the name and there is no registration to wait for");
